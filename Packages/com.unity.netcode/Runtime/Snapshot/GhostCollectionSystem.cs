...
namespace Unity.NetCode
{
    ...
    public class GhostCollectionSystem : SystemBase
    {
        ...
        private ulong HashGhostType(in GhostCollectionPrefabSerializer ghostType)
        {
            ...
            var a = new int[]{
                ghostType.FirstComponent,
                ghostType.NumComponents, // --
                ghostType.NumChildComponents, 
                ghostType.SnapshotSize, // --
                ghostType.ChangeMaskBits,  // --
                ghostType.PredictionOwnerOffset, 
                ghostType.OwnerPredicted, 
                ghostType.IsGhostGroup, 
            };
            ...
            for (int i = ghostSerializerCollection.Length; i < ghostCollection.Length; ++i)
            {
                ...
                if ((ghost.Hash != 0 && ghost.Hash != hash) || hash == 0)
                {
                    ...
                    if (hash == 0)
                        ...
                        UnityEngine.Debug.LogError($"Received a ghost from the server({ghost.Hash}) which has a different hash on the client({hash})");
#endif
                    Entities
