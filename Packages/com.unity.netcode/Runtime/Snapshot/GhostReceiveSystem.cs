...
namespace Unity.NetCode
{
    ...
    public unsafe class GhostReceiveSystem : SystemBase, IGhostMappingSystem
    {
        ...
        private EntityQuery playerGroup1;
            ...
            public unsafe void Execute()
            {
                ...
                if (numPrefabs > 0)
                {
                    ...
                    for (int i = 0; i < numPrefabs; ++i)
                    {
                        ...
                        {
        #if ENABLE_UNITY_COLLECTIONS_CHECKS || CUSTOM_DEBUG
...
#endif
            // UnityEngine.Debug.LogError(string.Format("Ghost Receive: player empty: {0}, player empty1: {1}, has networkingame {2}", 
            //     playerGroup.IsEmptyIgnoreFilter, 
            //     playerGroup1.IsEmptyIgnoreFilter,
            //     HasSingleton<NetworkStreamInGame>()));
        ...
        internal void AddNonSpawnedGhosts(NativeArray<NonSpawnedGhostMapping> ghosts)
        {
            ...
            for (int i = 0; i < ghosts.Length; ++i)
            {
                ...
                UnityEngine.Debug.LogError(string.Format("non-add World({1}): {0}", ent, World.Name));
        ...
        internal void UpdateSpawnedGhosts(NativeArray<SpawnedGhostMapping> ghosts)
        {
            ...
            for (int i = 0; i < ghosts.Length; ++i)
            {
                ...
                {
                    UnityEngine.Debug.LogError(string.Format("update-add World({1}): {0}", ent, World.Name));
                    m_ghostEntityMap[ghost.ghostId] =  ent;
                }
                if (!m_spawnedGhostEntityMap.TryAdd(ghost, ent))
                {
