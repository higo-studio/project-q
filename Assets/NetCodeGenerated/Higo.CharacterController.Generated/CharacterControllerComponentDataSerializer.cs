...
namespace Higo.CharacterController.Generated
{
    ...
    public struct CharacterControllerComponentDataGhostComponentSerializer
    {
        ...
        private static void PredictDelta(IntPtr snapshotData, IntPtr baseline1Data, IntPtr baseline2Data, ref GhostDeltaPredictor predictor)
        {
            ...
            snapshot.Looking_x = predictor.PredictInt(snapshot.Looking_x, baseline1.Looking_x, baseline2.Looking_x);
            snapshot.Looking_y = predictor.PredictInt(snapshot.Looking_y, baseline1.Looking_y, baseline2.Looking_y);
            snapshot.Jumped = predictor.PredictInt(snapshot.Jumped, baseline1.Jumped, baseline2.Jumped);
        }
        [BurstCompile]
        ...
        private static void Serialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamWriter writer, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                writer.WritePackedIntDelta(snapshot.Looking_x, baseline.Looking_x, compressionModel);
            if ((changeMask & (1 << 2)) != 0)
        ...
        private static void Deserialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamReader reader, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                snapshot.Looking_x = reader.ReadPackedIntDelta(baseline.Looking_x, compressionModel);
            else
                snapshot.Looking_x = baseline.Looking_x;
            if ((changeMask & (1 << 2)) != 0)
                snapshot.Looking_y = reader.ReadPackedIntDelta(baseline.Looking_y, compressionModel);
            else
                snapshot.Looking_y = baseline.Looking_y;
            if ((changeMask & (1 << 3)) != 0)
        ...
        private static void ReportPredictionErrors(IntPtr componentData, IntPtr backupData, ref UnsafeList<float> errors)
        {
            ...
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.Looking.x - backup.Looking.x));
            ++errorIndex;
        ...
        private static int GetPredictionErrorNames(ref FixedString512 names)
        {
            ...
            if (nameCount != 0)
                names.Append(new FixedString32(","));
            names.Append(new FixedString64("Looking.x"));
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32(","));
            names.Append(new FixedString64("Looking.y"));
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32(","));
