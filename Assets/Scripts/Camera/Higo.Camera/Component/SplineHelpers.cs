...
namespace Higo.Camera
{
    public static class SplineHelpers
    {
        ...
        public static float3 BezierTangent3(
            float t, float3 p0, float3 p1, float3 p2, float3 p3)
        {
            t = math.clamp(t, 0, 1);
        ...
        public static void ComputeSmoothControlPoints(
            in float4[] knot, ref float4[] ctrl1, ref float4[] ctrl2)
        {
        ...
        public static void ComputeSmoothControlPointsLooped(
            ref float4[] knot, ref float4[] ctrl1, ref float4[] ctrl2)
        {
            int numPoints = knot.Length;
            if (numPoints < 2)
            {
            ...
            float4[] knotLooped = new float4[numPoints + 2 * margin];
            float4[] ctrl1Looped = new float4[numPoints + 2 * margin];
            float4[] ctrl2Looped = new float4[numPoints + 2 * margin];
            ...
            ComputeSmoothControlPoints(in knotLooped, ref ctrl1Looped, ref ctrl2Looped);
            for (int i = 0; i < numPoints; ++i)
            {
