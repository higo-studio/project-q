...
namespace Beffio.Dithering
{
    ...
    public class PaletteEditor : Editor
    {
        ...
        private static class ContentText
        {
            ...
            public static GUIContent clearTextureButton = new GUIContent("Clear Texture");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    {
                        GeneratePaletteTexture(CurrentPalette);
                        BeffioDitherEditorUtilities.UpdateMaterialsForTexture(_palette.Texture2D);
                        CurrentPalette.IsDirty = false;
                    }
                    else
                    {
        ...
        private void DrawButtonsGUI()
        {
            ...
            if (GUILayout.Button(ContentText.clearTextureButton))
            {
                ClearPaletteTexture(CurrentPalette);
                CurrentPalette.IsDirty = true;
            }
        ...
        {
            Texture2D texture = CurrentPalette.Texture2D;
        ...
        private void GeneratePaletteTexture(Palette palette)
        {
            ...
            {
                palette.Texture = new Texture2D(_colorSquares * _colorSquares, height, TextureFormat.RGB24, false, true);
            ...
            {
                palette.Texture2D.Resize(_colorSquares * _colorSquares, height);
            }
            ...
            for (int i = 0; i < palette.Texture.height * palette.Texture.width; i++)
            {
                ...
                palette.Texture2D.SetPixel(x, y, Color.magenta);
            }
            ...
            for (x = 0; x < _colorSquares * _colorSquares; x++)
            {
                for (y = 0; y < _colorSquares; y++)
                {
                    ...
                    {
                        palette.Texture2D.SetPixel(x, y + (CurrentPalette.MixedColorCount - c - 1) * _colorSquares, palette.Colors[(int)mixingPlan[c]]);
                    }
                }
            }

            palette.Texture2D.Apply();
        ...
        private void ClearPaletteTexture(Palette palette)
        {
            var objs = AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(palette));
